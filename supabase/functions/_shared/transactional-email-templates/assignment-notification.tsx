...
const AssignmentNotificationEmail = (props: Props) => (
  <Html lang="en" dir="ltr">
    <Head />
    <Preview>{`You\u2019ve been assigned a ${props.enquiryType || 'new'} enquiry from ${props.clientName || 'a client'}`}</Preview>
    <Body style={main}>
      <Container style={container}>
        <Img src={LOGO_URL} alt="WCI Wallpapers" width="120" height="auto" style={logo} />
        <Hr style={divider} />
        <Heading style={h1}>
          Hi {props.assigneeName || 'there'},
        </Heading>
        <Text style={intro}>
          You{"\u2019"}ve been assigned a new <strong>{props.enquiryType || 'enquiry'}</strong> from{' '}
          <strong>{props.clientName || 'a client'}</strong>.
        </Text>

        <Hr style={lightDivider} />
        <Text style={sectionLabel}>Client Details:</Text>
        <Section style={detailsSection}>
          <DetailRow label="Name" value={props.clientName} />
          <DetailRow label="Email" value={props.clientEmail} />
          <DetailRow label="Phone" value={props.clientPhone} />
          <DetailRow label="Company" value={props.clientCompany} />
          <DetailRow label="Role" value={props.clientRole} />
          <DetailRow label="Location" value={props.clientLocation} />
        </Section>

        {(props.projectName || props.projectLocation || props.projectStage || props.requestType) && (
          <>
            <Hr style={lightDivider} />
        ...
        {(props.message || props.projectNotes) && (
          <>
            <Hr style={lightDivider} />
            <Text style={sectionLabel}>{props.message ? 'Message:' : 'Notes:'}</Text>
            <Text style={messageText}>{props.message || props.projectNotes}</Text>
          </>
        )}
...
export const template = {
  ...
  subject: (data: Record<string, any>) =>
    `New assignment: ${data.enquiryType || 'enquiry'} from ${data.clientName || 'a client'}`,
  displayName: 'Team member assignment notification',
  previewData: {
...
const h1 = { fontSize: '22px', fontWeight: '400' as const, color: '#1C1C1C', lineHeight: '1.4', margin: '0 0 8px' }
const intro = { fontSize: '14px', color: '#1C1C1C', lineHeight: '1.7', margin: '0 0 4px' }
